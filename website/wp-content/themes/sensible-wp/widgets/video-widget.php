...
// constructor
    ...
    function sensiblewp_video_widget() {
		...
		$widget_ops = array('classname' => 'sensiblewp_video_widget_widget', 'description' => __( 'Video for your sidebar.', 'sensible-wp') );

        parent::WP_Widget(false, $name = __('MT: Video Widget', 'sensible-wp'), $widget_ops); 
	...
	<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'Paste the URL of the video (only from a network that supports oEmbed, like Youtube, Vimeo etc.):', 'sensible-wp' ); ?></label>
	...
	function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Video', 'sensible-wp' );
