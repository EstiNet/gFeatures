                    ...
                    if (OrangeTeam.hasPlayer(event.getPlayer())) {
                        if (point.getCaptureState().equals(CaptureState.blue) || point.getCaptureState().equals(CaptureState.white)) {
                            ...
                            for (Player p : Bukkit.getOnlinePlayers()) {
                                p.sendTitle(new Title(ChatColor.GOLD + point.getName() + " is being captured!", ChatColor.GOLD + point.getName() + " is now " + ChatColor.WHITE + ChatColor.BOLD + " NEUTRAL.", 20, 40, 20));
                            }
                            Bukkit.getScheduler().scheduleSyncDelayedTask(Bukkit.getServer().getPluginManager().getPlugin("gFeatures"), new Runnable() {
                                public void run() {
                                    ...
                                    if (point.isInLocation(event.getPlayer().getLocation())) {
                                        ...
                                        s.flushAll();
                                        for (Player p : Bukkit.getOnlinePlayers()) {
                                            p.sendTitle(ChatColor.GOLD + point.getName() + " has been captured by Kloyne!", ChatColor.GOLD + point.getName() + " is now ORANGE.", 20, 40, 20);
                                        }
                        ...
                        if (point.getCaptureState().equals(CaptureState.orange) || point.getCaptureState().equals(CaptureState.white)) {
                            ...
                            Constants.capturetrigger.add(event.getPlayer().getName());
                            for (Player p : Bukkit.getOnlinePlayers()) {
                                p.sendTitle(new Title(ChatColor.DARK_AQUA + point.getName() + " is being captured!", ChatColor.DARK_AQUA + point.getName() + " is now " + ChatColor.WHITE + ChatColor.BOLD + " NEUTRAL.", 20, 40, 20));
                            }
                            Bukkit.getScheduler().scheduleSyncDelayedTask(Bukkit.getServer().getPluginManager().getPlugin("gFeatures"), new Runnable() {
                                public void run() {
                                    Constants.capturetrigger.remove(event.getPlayer().getName());
                                    if (point.isInLocation(event.getPlayer().getLocation())) {
                                        ...
                                        s.flushAll();
                                        for (Player p : Bukkit.getOnlinePlayers()) {
                                            p.sendTitle(new Title(ChatColor.DARK_AQUA + point.getName() + " has been captured by Innisfil!", ChatColor.DARK_AQUA + point.getName() + " is now BLUE.", 20, 40, 20));
                                        }
                                    }
                                }
                            }, 60L);
